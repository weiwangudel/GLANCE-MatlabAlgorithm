...
for i = 0:size(fs_dirpart_32,1)
    ...
    if (~isempty(indx))
        dtarr(i+1) = max(fs_filepart_32(indx,dtindx));
    else
        dtarr(i+1) = 0;
    end;
end;
...
while (size(resT,1) > 0)
    ...
    for i = 1:size(resT,1)
        ...
        if (dtarr(curd+1) > thresh)
            ...
            res = [res; indx(find(fs_filepart_32(indx,dtindx) > thresh))];
        end;
...
dtm = [dtm; datem]
bpe = [bpe; sum(fs_filepart_32(res,dtindx) > thresh)]

end;

realres = sum(fs_filepart_32(:,dtindx) > thresh)
accuracy = mean(bpe)/realres
dir_covered = mean(dtm)
